...
- input:  data/enscgp_posterior.npy  (N, 5, 200, 200) = [u, v, L11, L21, L22],
          the EnsCGP first guess (enscgp_train.py) being refined.
...
All five weights are configurable ("training" section: nll_weight/l1_weight/
spectral_weight/gradient_weight/quantile_weight, all defaulting to 1.0 for the first two
and 0.0 -- i.e. off -- for the new three, so introducing them does not change existing
runs until explicitly enabled). The Gaussian NLL is the only term that uses the predicted
covariance (chol_head); spectral/gradient/quantile are computed exclusively on the
predicted mean (mu_u, mu_v) vs. wrf_uv, adapted from the WeightedWindLoss losses in
26.3_wind/SWIN/train_wind_swin2sr.py (that script's L1 there is folded into the L1 term
above instead of being duplicated; see MeanAuxLosses below for spectral/gradient/quantile).
...
index-aligned with each other and with that split file).
...
def evaluate(model, loader, terrain_raw, device, weights: dict, mean_aux: MeanAuxLosses) -> dict:
    ...
    for posterior, wrf in loader:
        ...
        pred = model(posterior, terrain_raw)
...
def train(config: dict, device_str: str, resume_path: Path | None = None, max_steps: int | None = None):
    ...
    train_ds = EnsCGPSwinDataset(posterior_path, wrf_path, train_idx)
    val_ds = EnsCGPSwinDataset(posterior_path, wrf_path, val_idx)
