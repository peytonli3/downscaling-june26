...
uses eval_new_enscgp_swin_checkpoint.py's speed_zscore: a delta-method (first-order
...
For each of --n_samples WRF/EnsCGP-posterior/bicubic triples run through a trained
ProbabilisticSwin2SR checkpoint (forward(posterior, bicubic, terrain_raw), see
new_enscgp_swin.py), z_speed/z_u/z_v are computed at every one of the
200x200 pixels and pooled (no masking) into one array per component; each pooled array
is binned into its own histogram panel and plotted against the N(0, 1) PDF, alongside
the fraction of pixels within +/-1 and +/-2 sigma versus the 68.3%/95.4% a perfectly
calibrated model would have.

Sample selection mirrors eval_new_enscgp_swin_checkpoint.py: by default drawn from the
...
Usage:
    python eval_zscore_calibration.py --checkpoint /home/peytonli/26.6_wind/logs/checkpoints/best.pth
...
from train_new_enscgp_swin import DEFAULT_CONFIG_PATH, build_model, load_config  # noqa: E402
...
def speed_zscore(mu_u: np.ndarray, mu_v: np.ndarray, var_u: np.ndarray, var_v: np.ndarray, cov_uv: np.ndarray,
    ...
    eval_new_enscgp_swin_checkpoint.py -- see that module's docstring for the
...
def collect_zscores(idx: np.ndarray, posterior, bicubic, wrf, model, terrain_raw, device, batch_size: int) -> dict:
    ...
    for start in range(0, len(idx), batch_size):
        ...
        bicubic_batch = torch.from_numpy(np.array(bicubic[batch_idx], dtype=np.float32, copy=True)).to(device)
        with torch.no_grad():
            pred_batch = model(posterior_batch, bicubic_batch, terrain_raw).cpu().numpy()  # (b, 5, H, W)
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=None,
                         help="Defaults to inference_results/zscore_histograms.png, or "
                              ".../zscore_histograms_top<top_pct>pct_<magnitude_source>.png if --top_pct is set")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    args = parser.parse_args()

    if args.output is None:
        if args.top_pct is None:
            args.output = Path("/home/peytonli/26.6_wind/inference_results/0701/zscore_histograms.png")
        else:
            args.output = Path(
                f"/home/peytonli/26.6_wind/inference_results/0701/"
                f"zscore_histograms_top{args.top_pct:g}pct_{args.magnitude_source}.png"
            )
    ...
    z = collect_zscores(idx, posterior, bicubic, wrf, model, terrain_raw, device, args.batch_size)
