...
via select_regularization_gcv from extra_scripts/ken_enscgp/utils.py.

Context: tune_enscgp_sigma.py finds sigma by bisecting for a target posterior
...
terms of sigma (std) -- tune_enscgp_sigma.py, enscgp_train.py's --sigma_mean/
...
target SSR==1 the way tune_enscgp_sigma.py does.

Usage:
    python tune_enscgp_lambda_gcv.py --split val --n_samples 150
"""
import argparse
import sys
...
sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "ken_enscgp"))
