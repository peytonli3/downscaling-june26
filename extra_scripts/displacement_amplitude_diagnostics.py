...
Reuses (does not reinvent):
  ...
  - DX_KM/DY_KM/PIXEL_KM, scale_labels, generate_predictions from
    extra_scripts/band_error_diagnostics.py -- same pixel->km scale, same checkpoint/data
    loading (post-re-registration WRF, logs/0701 checkpoint), same held-out split.
...
SCRIPTS_DIR = Path("/home/peytonli/26.6_wind/scripts")
EXTRA_SCRIPTS_DIR = Path("/home/peytonli/26.6_wind/extra_scripts")
for p in (SCRIPTS_DIR, EXTRA_SCRIPTS_DIR):
...
OUTPUT_DIR = Path("/home/peytonli/26.6_wind/inference_results")
COMPONENTS = ("u", "v")
...
def main() -> None:
    ...
    print_summary(rows)
    write_csv(rows, OUTPUT_DIR / "0701" / f"{args.output_prefix}.csv")
    plot_decomposition_bar(rows, S, OUTPUT_DIR / "0701" / f"{args.output_prefix}_bars.png")
    plot_shift_quivers(rows, args.n_levels, OUTPUT_DIR / "0701" / f"{args.output_prefix}_quiver.png")
    plot_displacement_vs_scale(rows, OUTPUT_DIR / "0701" / f"{args.output_prefix}_vs_scale.png")
