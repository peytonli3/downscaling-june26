...
1. Predictions: load a trained ProbabilisticSwin2SR checkpoint (scripts/new_enscgp_swin.py)
   and run it on the held-out test split, exactly as eval_new_enscgp_swin_checkpoint.py
...
Outputs (to inference_results/): a CSV + printed table (rows coarse->fine) and a PNG
...
# (extra_scripts/graphing/swin/compare_eigenspectra_enscgp_swin.py). Anisotropic: a degree
...
def generate_predictions(args) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    checkpoint = args.checkpoint or (Path(paths["log_dir"]) / "checkpoints" / "best.pth")
